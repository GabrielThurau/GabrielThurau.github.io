...
function addStyle(styleString) {
  ...
  // checking for just liver products
  
    function checkCart(cart, injectionZone) {
       let cartItems = cart.items;
       let item = cartItems.find(product => product.handle.indexOf('liver') > -1 && !product?.selling_plan_allocation);
       let otpLiverProducts = cartItems.filter(product => product.handle.indexOf('liver') > -1 && !product?.selling_plan_allocation);
       if (otpLiverProducts.length !== 0) {
        let itemKey = item?.key;
       if (!item?.selling_plan_allocation ) {
        let VID = item?.variant_id;
         let twoOunce = cartItems.find(product => product.variant_id === 34643863208099 && !product?.selling_plan_allocation);
         // multiple values to account for whenever it's the two ounce liver product
         if (twoOunce && cartItems[0].variant_id === 34643863208099
            || cartItems[0].variant_id !== 34643863208099 
            && cartItems[0].handle.indexOf('liver') > -1
            && cartItems[0]?.selling_plan_allocation ) {
        console.log('two ounce!')
      ...
      // do fetches function - goal is to combine both clearItems() and postItems() functions into one operation
