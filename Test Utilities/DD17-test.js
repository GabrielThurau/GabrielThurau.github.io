...
function addStyle(styleString) {
    ...
    const generateMarkup = (twoOunce, injectionZone) => {
        let markup =  
        `
        ...
        <div class="topmost-wrapper">
          ...
          <div class="copy-container">
            <h3 class="subscribe-copy">Subscribe and Save!</h3>
          </div>
        </div>
        <div class="text-section">
          ...
          <ul class="sub-steps">
          ${twoOunce?.variant_id === 34643863208099 && twoOunce
            ?
    ...
    if (injectionZone) {
      ...
      checkCart2(cart, injectionZone)
    }
    }
    
      // checking for just liver products

    function checkCart2(cart, injectionZone, markupOne, markupTwo) {
        ...
        let itemKey = item.key;
          let VID = item.variant_id;
           let twoOunce = liverProducts.find(product => product.variant_id === 34643863208099);
           // multiple values to account for whenever it's the two ounce liver product
           if (twoOunce) {
          generateMarkup(twoOunce, injectionZone);
