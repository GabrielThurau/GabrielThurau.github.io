...
.product-image-wrap > img {
   ...
   div.CartDrawer_drawer-cart-box__LK78a > div:nth-child(2) > button {
   ...
   .CartDrawer_drawer-cart-box__LK78a:has(div.button-container) {
...
const addMarkup = () => {
    ...
    let injectedHeading = document.querySelector('.end-boring-workout');
    let buttonContainer = document.querySelector('.button-container');
    let emptyCartSection = document.querySelector('.empty-cart-sect');
    let cartText = document.querySelector("div.CartDrawer_drawer-cart-box__LK78a > div:nth-child(1)");
    let cartCount = document.querySelector('.CartDrawer_cartDrawerMenu__main__header__title__UP4NG').textContent;
    if (!el.classList.contains('av14') && cartText && cartCount === 'YOUR CART (0)') {
    ...
    `
           if (!emptyCartSection) {
            ...
            if (!buttonContainer) {
               document.querySelector("div.CartDrawer_drawer-cart-box__LK78a > div:nth-child(2)").insertAdjacentHTML('afterend', twoButtons);
            }
    }
}

const findEmptyThing = () => {
  let cartCount = document.querySelector('.CartDrawer_cartDrawerMenu__main__header__title__UP4NG');
    el = document.querySelector('#cart-drawer-menu');
    if (el && el.children.length === 0 ) {
        clearInterval(doFindThing);
        doFindThing = setInterval(findThing, 50);
    }

     else if (el.children.length > 0 && cartCount &&cartCount.textContent === 'YOUR CART (0)') {
        clearInterval(doFindThing);
        doFindThing = setInterval(findThing, 50);
        }
};
...
const findThing = () => {
    ...
    let cartCount = document.querySelector('.CartDrawer_cartDrawerMenu__main__header__title__UP4NG');
     let cartText = document.querySelector("div.CartDrawer_drawer-cart-box__LK78a > div:nth-child(1)");
    if (el && el.children.length > 0 && cartText) {
        clearInterval(doFindThing);
        cartCount = cartCount.textContent;
        addMarkup();
        doFindThing = setInterval(findEmptyThing, 50);
    }
};
let doFindThing = setInterval(findThing, 50);